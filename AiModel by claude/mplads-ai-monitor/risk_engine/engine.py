...
def band_of(score: float, cfg) -> str:
    ...
    Assignment is by lower bound only. The configured upper bounds are integers
    for readability, so testing `lo <= score <= hi` would drop a score of 74.6
    into no band at all — the kind of silent gap that quietly mis-labels records.
    """
    bands = cfg["risk_engine.bands"]
    for name in ("critical", "high", "moderate"):
        if score >= bands[name][0]:
            return name.upper()
    return "LOW"
...
def _bands(scores: pd.Series, cfg) -> pd.Series:
    """Vectorised banding — lower bound only, for the reason given in band_of()."""
    bands = cfg["risk_engine.bands"]
    out = pd.Series("LOW", index=scores.index)
    for name in ("moderate", "high", "critical"):
        out[scores >= bands[name][0]] = name.upper()
    return out
...
def score_works(features: pd.DataFrame, anomalies: pd.DataFrame,
    ...
    df = compute_work_components(df, cfg)
    return finalise_work_scores(df, cfg)


# The component maths lives in ONE function so that the batch pipeline and the
# live scoring API (backend/services/scoring.py) cannot drift apart: both call
# compute_work_components() followed by finalise_work_scores() on a frame with
# the same column names.
WORK_COMPONENTS = ["cost_risk", "duplicate_risk", "delay_risk", "vendor_risk",
                   "utilisation_risk", "data_quality_risk"]

# Inputs the component maths reads. Anything absent is treated as unknown (NaN),
# which the ramps turn into a zero contribution rather than a guess.
WORK_COMPONENT_INPUTS = [
    "state_category_robust_z", "state_category_ratio_to_peer", "district_robust_z",
    "peer_group_thin", "top_duplicate_similarity", "duplicate_partner_count",
    "duplicate_same_mp", "description_repeat_same_mp", "work_stage", "open_age_ratio",
    "district_vendor_hhi", "mp_top_vendor_share", "mp_repeat_payment_share",
    "mp_unspent_pct", "mp_pending_payment_share", "mp_published_vs_derived_gap_pct",
    "missing_data_ratio", "description_length", "is_repeated_work_id",
]


def compute_work_components(df: pd.DataFrame, cfg=None) -> pd.DataFrame:
    """Compute the six explainable 0-100 components for a frame of works."""
    cfg = cfg or get_config()
    df = df.copy()
    for col in WORK_COMPONENT_INPUTS:
...
def finalise_work_scores(df: pd.DataFrame, cfg=None) -> pd.DataFrame:
    """Combine the components into the composite, band, contributions and explanation."""
    cfg = cfg or get_config()
    w = cfg["risk_engine.weights"]
    df = df.copy()
    df["composite_risk"] = combine(df, WORK_COMPONENTS, w, cfg)
    ...
    df["contributions"] = [
        json.dumps({c: round(float(r[c]) * w[c], 2) for c in WORK_COMPONENTS} |
