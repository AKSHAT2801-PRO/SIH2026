...
15a. [Scoring a new record](#15a-scoring-a-record-that-is-not-in-the-warehouse)
...
| POST | `/api/score` | **score a record that is not in the warehouse** (see below) |
| POST | `/api/score/batch` | score up to 200 submitted records in one call |
| GET | `/api/score/schema` | the scoreable-record contract and the reference run it scores against |
...
## 15a. Scoring a record that is not in the warehouse

Full reference: [`docs/SCORING_API.md`](docs/SCORING_API.md).

`POST /api/score` answers the question the batch register cannot: an implementing
agency proposes a work — does it look unusual against comparable published
records? Only `state` and `amount` are required.

```bash
curl -s -X POST http://localhost:8000/api/score \
  -H 'content-type: application/json' -H 'x-api-key: dev-analyst-key' \
  -d '{"state":"Uttar Pradesh","amount":4939000,
...
The response carries the composite score and band, the six components, the
weighted contributions, the written explanation, the peer evidence (peer median,
ratio, robust z, similar published works, comparable works), the unsupervised
corroboration, whether it would raise an alert, what could **not** be assessed,
and the reference run every number came from.

**How one record can be scored at all.** A score is a statement relative to
comparable records, so every pipeline run freezes the reference population into a
versioned *scoring pack* (`outputs/models/scoring_pack.joblib`): peer medians and
MADs per (state × category), district and category; district vendor
concentration; per-MP context; the median age of completed works; the fitted
Isolation Forest; and the propensity model. Duplicate detection runs live against
the warehouse with the same character n-gram TF-IDF method as the batch run. With
no completed run on disk the endpoint returns 503 rather than inventing a
baseline.

**Equivalence with the register.** The service does not re-implement the risk
maths — it builds a one-row frame with the batch pipeline's own column names and
calls the same `compute_work_components` → `finalise_work_scores` pair. A test
re-scores published works through the live path and asserts every component is
within 5 points and the composite within 3; measured drift on the current run is
0.00–0.40 points. Two differences are reported honestly in each response: LOF and
DBSCAN cannot run on a single point (their weight falls back to the Isolation
Forest score, exactly as it does for batch rows outside the LOF sample), and the
record is not part of its own peer group.

`?persist=true` (analyst role) also writes the scored record into
`analytics_work_risk` and raises an alert if it crosses the threshold.
...
python scripts/build_scoring_pack.py      # refresh the live-scoring reference pack on its own
python scripts/score_record.py examples/new_work.json        # score a proposed work
python scripts/score_record.py examples/new_works_batch.json # or several at once
...
```bash
MPLADS_AUTH_DISABLED=1 pytest tests -q
```
...
* **Live-scoring tests** — a record missing every optional field still scores and says what it
  could not assess; required fields are enforced; scoring is deterministic; no response asserts
  fraud; a grossly overpriced duplicate outranks an ordinary work; a bad record in a batch is
  reported rather than fatal; and **re-scoring published works through the live path reproduces
  the batch register** (every component within 5 points, composite within 3).
* **Banding regression** — no score may fall between two bands: the configured edges are integers,
  so 49.6 and 74.6 must land in MODERATE and HIGH rather than defaulting to LOW.
