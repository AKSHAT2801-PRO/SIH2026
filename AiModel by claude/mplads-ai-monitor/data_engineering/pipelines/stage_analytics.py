...
from risk_engine.scoring_pack import build_scoring_pack
...
def run(ctx: RunContext, works: pd.DataFrame, expenditures: pd.DataFrame,
    ...
    # ------------------------------------------- 3. anomaly ensembles
    fitted: dict = {}
    work_anom = ensemble(work_features, WORK_MODEL_FEATURES, "works", collect=fitted)
    ...
    # ------------------------------------ 8. scoring pack for the live API
    # Freezes the reference population (peer statistics, district concentration,
    # MP context, the fitted Isolation Forest and the propensity model) so a
    # record that is not in the warehouse can be scored on the same footing.
    pack = build_scoring_pack(
        run_id=ctx.run_id, works=works, expenditures=expenditures, mp_features=mp_features,
        snapshot_ts=works["event_date"].max(),
        iforest_artifact=fitted.get("isolation_forest"),
        propensity_model=prediction.pop("_model", None),
        propensity_columns=prediction.pop("_columns", None),
    )
    ctx.event("scoring_pack", "built", run_id=pack.run_id, mps=len(pack.mp_context),
              peer_cells=len(pack.state_category))
